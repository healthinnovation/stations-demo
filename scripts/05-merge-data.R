...
final_path <- paste(out_path, "stations.csv", sep = "/")

if (!dir.exists(fs::path_dir(final_path))) {
  dir.create(fs::path_dir(final_path), recursive = TRUE)
}

dataset_app <- dplyr::bind_rows(weather_long, quality_long)
readr::write_csv(dataset_app, final_path)
print(final_path)
